...
class Address extends FormInput
{
    ...
    protected function hasError(?int $langID = null): bool
    {
        ...
        $hasHiddenLocation = !empty($_POST[ 'latitude-' . $this->fieldName ])
            && !empty(
            $_POST[ 'longitude-' . $this->fieldName ]
            );
        $hasLocation       = !empty($_POST['latitude']) && !empty($_POST['longitude']);
        if ($postValue == null && $this->isRequired) {
            return _('Campo obligatorio.');
        } else {
            if (!$hasHiddenLocation && !$hasLocation) {
                return _('Asegúrate de haber creado un marcador en el mapa con la dirección correcta.');
            }
        }
        return false;
    }
    ...
    private function getPosition(): array|false
    {
        $sql      = '
