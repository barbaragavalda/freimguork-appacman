...
class Permissions extends FormInput {
    ...
    protected function getInputHTML($langID = null){
        ...
        if( !$this->isSuperAdmin && $unmodifiable ){
            return gettext('No se pueden modificar los permisos de los administradores.');
        }
            ...
            $form .= '
                <div class="form-horizontal lang_2">
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <select name="'.$this->fieldName.'_'.$content['id'].'[]"  class="form-control select2 select2-hidden-accessible" multiple="" data-placeholder="'.gettext('Selecciona').' '.$this->getPlaceholder().'" style="width: 100%;" tabindex="-1" aria-hidden="true">
    ...
    public function save($itemID, $langID = null){
        ...
        if( $this->mysql->getState() ){
            ...
            foreach($this->contents as $content) {
                ...
                $params['id_appacman_content_'.$contentID] = array('value'=> $contentID, 'type' => \PDO::PARAM_INT);
                $postName = $this->fieldName . '_' . $contentID;
                if (isset($_POST[$postName])) {
                    foreach($_POST[$postName] as $index => $id){
                        $values[] = '(:id_appacman_user_profile, :id_appacman_content_'.$contentID.', :id_'.$contentID.'_'.$index.')';
    ...
    private function initRefereneProfile(){
    ...
    private function getOptionsHTML($contentID){
        $sql = '
    ...
    private function loadValues($contentID){
        if( isset($_POST['save']) ){
            if( isset($_POST[$this->fieldName.'_'.$contentID]) ) {
                return $_POST[$this->fieldName . '_' . $contentID];
            }else{
                return array();
            }
        }
