...
class UserForm extends Form
{
    ...
    public function signin(): void
    {
        ...
        if (!empty($this->form['user']) && !empty($this->form['password'])) {
            ...
            $this->error = _('Debes llenar todos los campos obligatorios.');
        }
    }
    ...
    private function setupChangePassword(): bool
    {
        ...
        if ($this->foundUser()) {
            ...
            if ($this->mysql->rowCount()) {
                ...
                $this->error = _('Se ha producido un error al enviar el email. Por favor, inténtalo más tarde.');
            }
        } else {
            $this->error = _('No existe ningún usuario con este email.');
        }
    ...
    private function sendMailChangePassword($hash): bool
    {
        ...
        $link  = $domain . _('cambiar-contrasena') . '/' . $hash;
        $title = _('Haz click para cambiar tu contraseña');
        return $mail->send(
            array('email' => $mailConfig['username'], 'name' => $mailConfig['name']),
            array(array('email' => $userEmail, 'name' => $userName)),
            $businessInfo['name'] . ' - ' . _('Recordar contraseña'),
            "$name <div><a href='$link' style='display:inline-block; text-decoration:none; text-transform: uppercase; text-size: 16px; padding:6px 30px; background:#39CCCC; color:#ffffff; border: 1px solid #36a8a9;'>$title</a></div>"
        );
    }
    ...
    public function canChange($hash): bool
    {
        $sql    = '
        ...
        if (!empty($this->form['password1']) && !empty($this->form['password2'])) {
            if ($this->form['password1'] === $this->form['password2']) {
                ...
                if ($this->mysql->rowCount()) {
                    ...
                    $this->error = _('Se ha producido un error en el servidor. Por favor, inténtalo más tarde.');
                }
            } else {
                $this->error = _('Las contraseñas no coinciden.');
            }
        } else {
            $this->error = _('Debes llenar todos los campos obligatorios.');
        }
