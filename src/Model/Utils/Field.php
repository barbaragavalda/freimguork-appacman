...
class Field extends Model {
    ...
    public function getFieldsForExport(){
        $names = array();
        foreach($this->fields as $field){
            if( !in_array($field['type'], array('unmodifiable', 'encryptedOneWay', 'timestamp', 'image', 'imageSeeOnly', 'genericFile', 'genericFileSeeOnly')) ){
                $names[] = $field;
            }
        }
    ...
    private function init(){
        $sql = '
            SELECT af.id_appacman_field, af.field_name, af.show_on_list, af.show_on_breadcrumb, afl.name, aft.name AS type
